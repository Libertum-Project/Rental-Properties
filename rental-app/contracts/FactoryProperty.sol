...
contract FactoryProperty is AccessControl {
    ...
    //~~~~ IMMUTABLE VARIABLES ~~~~
    Bank immutable private s_bank; //bank will be deployed from this contract to keep it connected as a deployer.
    ...
    function createNewPropertyLoan(string memory _name, string memory _symbol, uint256 _totalSupply, uint256 _initialPricePerPiece, uint256 _valueBackup, uint256 _duration, uint256 _totalRate,  address _receiver) external onlyRole(CREATOR_ROLE) {        
            _registerCollection(address(_createPropertyLoan(_name, _symbol, _totalSupply, _initialPricePerPiece, _valueBackup, _duration, _totalRate, _receiver)));
    }
    ...
    function _createPropertyLoan(string memory _name, string memory _symbol, uint256 _totalSupply, uint256 _initialPricePerPiece, uint256 _valueBackup, uint256 _duration, uint256 _totalRate, address _receiver) internal returns(address){
        //creation of new erc1155(property)
        PropertyLoan _newProperty = new PropertyLoan(_name, _symbol, _totalSupply, _initialPricePerPiece, _valueBackup, _duration, _totalRate, _receiver, bankAddress());
